...
def compute_metrics(gt_sample: np.ndarray, pred_sample: np.ndarray, threshold=0.01):
    """
    计算给定 GT 和 Pred 的所有基因通道下的 RMSE、PSNR、SSIM、CC、IFC和VIF，并返回各通道平均值。
    ...
    return rmse_avg, psnr_avg, ssim_avg, cc_avg, ifc_avg, vif_avg
...
def main():
    ...
    for model_dir in model_dirs:
        ...
        results_dir = os.path.join(root_dir, "brainresultnew", script_name)
        ...
        rmse_all, psnr_all, ssim_all, cc_all, ifc_all, vif_all = 0, 0, 0, 0, 0,0  # 添加IFC和VIF累计变量
        ...
        for batch_idx in range(num_batches):
            ...
            for j in range(hr.shape[0]):
                ...
                # 计算指标
                rmse_avg, psnr_avg, ssim_avg, cc_avg, ifc_avg, vif_avg= compute_metrics(gt_sample, pred_sample, threshold=0.1)
                from sklearn.metrics.pairwise import cosine_similarity
                gt_binary = (gt_sample > 0).astype(np.float32)
                pred_binary = (pred_sample > 0).astype(np.float32)
                pred_binary_1 = (pred_sample_1 > 0).astype(np.float32)
                
                cc_avg_sp_bi = cosine_similarity(gt_binary.flatten().reshape(1, -1), pred_binary.flatten().reshape(1, -1))[0][0]
                cc_avg_sp_no = cosine_similarity(gt_sample.flatten().reshape(1, -1), pred_sample.flatten().reshape(1, -1))[0][0]
                cc_avg_bi = cosine_similarity(gt_binary.flatten().reshape(1, -1), pred_binary_1.flatten().reshape(1, -1))[0][0]
                cc_avg_no = cosine_similarity(gt_sample.flatten().reshape(1, -1), pred_sample_1.flatten().reshape(1, -1))[0][0]
                print('cc_avg_sp_bi',cc_avg_sp_bi)
                print('cc_avg_sp_no',cc_avg_sp_no)
                print('cc_avg_bi',cc_avg_bi)
                print('cc_avg_no',cc_avg_no)



                # if average_sparsity > 0.9:
                #     cc_avg = 0
                rmse_batch += rmse_avg
                ssim_batch += ssim_avg
                cc_batch += cc_avg
            ...
            with open(progress_csv, "a") as f:
                f.write(f"{batch_idx+1},{rmse_avg},{ssim_avg},{cc_avg},{ifc_avg},{vif_avg},{psnr_all}\n")

            logger.log(f"批次 {batch_idx+1} 完成: RMSE={rmse_avg:.4f}, SSIM={ssim_avg:.4f}, CC={cc_avg:.4f}, IFC={ifc_avg:.4f}, VIF={vif_avg:.4f}, PSNR={psnr_avg:.4f}")
