...
def main():
    ...
    args.batch_size = 1
    args.SR_times = 10
    args.dataset_use = 'Xenium'
    
    # 文件路径统一采用训练时的路径（根据实际情况修改）
    gene_order_path = '/date/NMI/data/Xenium_pancreascancer/gene_order.npy'
    gene_name_order_path = '/date/NMI/data/Xenium_pancreascancer/gene_order_name.txt'
    
    # 查找所有符合条件的训练模型目录（注意：训练时日志保存在 logs5K1/ 目录下）
    model_dirs = glob.glob(os.path.join("logs5K1", f"{args.dataset_use}_{args.SR_times}X_G*"))
    ...
    for model_dir in model_dirs:
        ...
        data_num = 84 if args.dataset_use == 'Xenium' else 14
...
def load_superres_data(batch_size,data_root, dataset_use, status, SR_times, gene_num, all_gene, gene_order, gene_name_order):
    """
