...
class Xenium_dataset(Dataset):
    def __init__(self, data_root, SR_times, status, gene_num, all_gene, gene_order, gene_name_order):
        ...
        # 加载BERT模型，提取基因名称特征
        self.tokenizer = AutoTokenizer.from_pretrained("/date/NMI/bert", local_files_only=True, trust_remote_code=True)
        self.model = AutoModel.from_pretrained("/date/NMI/bert", local_files_only=True, trust_remote_code=True)
        self.model.eval()
        ...
        metadata_prompt = "Provide spatial transcriptomics data from the Xenium platform for human species, with a cancer condition, and pancreas tissue type."
        with torch.no_grad():
